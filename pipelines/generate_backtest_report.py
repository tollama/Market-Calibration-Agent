"""Backtest report generation with expanding walk-forward evaluation."""
...
def build_walk_forward_splits(
    ...
    initial_target = max(
        int(len(work) * float(cfg.initial_train_fraction)),
        int(cfg.min_train_rows),
    )
    start_time_pos = 0
...
def generate_backtest_report(
    ...
    *,
    ...
    summary_markdown = _render_summary_markdown(summary, overall, wf_overall)
...
def _build_prediction_export(frame: pd.DataFrame, variants: Mapping[str, str]) -> pd.DataFrame:
    columns = [column for column in ("market_id", "event_id", "ts", "resolution_ts", "label") if column in frame.columns]
...
def _render_summary_markdown(
    ...
    if overall.empty:
        lines.append("- No overall rows.")
    else:
        lines.extend(_markdown_table(overall))
    lines.extend(["", "## Walk-Forward Overall Summary", ""])
    if wf_overall.empty:
        lines.append("- Walk-forward report not generated.")
    else:
        lines.extend(_markdown_table(wf_overall))
    return "\n".join(lines).rstrip() + "\n"
